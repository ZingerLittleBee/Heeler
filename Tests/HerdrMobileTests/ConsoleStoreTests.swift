...
struct ConsoleStoreTests {
    ...
    @Test func renameWorkspaceThrowsWhenTheHostIsUnknown() async throws {
        let host = Host.fixture()
        let store = makeStore(transports: [:])
    ...
    @Test func renameWorkspaceFailureLeavesTheWorkspaceUntouched() async throws {
        let host = Host.fixture()
        let transport = ScriptedTransport(
            snapshot: .fixture(workspaces: [.fixture(workspaceID: "w1", label: "Old")]))
        let store = makeStore(transports: [host.id: transport])
        ...
        #expect(store.workspaces(for: host.id).map(\.label) == ["Old"])
        #expect(await transport.workspaceRenames.isEmpty)
