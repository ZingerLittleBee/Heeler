...
actor EventsSession {
    ...
    private func ensureTransport() async throws -> any Transport {
        ...
        do {
            try await measureLatency(on: transport)
        } catch {
            try? await transport.close()
            throw error
        }
    ...
    private func startKeepalive(stream: HerdrEventStream) {
        ...
        keepaliveTask = Task {
            while !Task.isCancelled {
                ...
                do {
                    try await self.measureLatency(on: transport)
    ...
    private func measureLatency(on transport: any Transport) async throws {
        let started = ContinuousClock.now
        _ = try await transport.ping()
        latencyContinuation.yield(started.duration(to: .now))
    }
