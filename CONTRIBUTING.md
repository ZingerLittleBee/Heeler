...
Commit messages and PR titles/descriptions carry no attribution trailers
and no email addresses.

- A Lefthook `commit-msg` hook blocks `Co-authored-by`, `Generated by` /
  `Generated with`, `🤖 Generated`, `Assisted-by`, and any email address on
  `git commit`. Install it once with `brew install lefthook && lefthook
  install` (the repo root has no `package.json` on purpose).
- CI (`.github/workflows/check-commit-attribution.yml`) scans every commit
  on the PR and the PR title/description with the same matchers.
- `git commit --no-verify` or `LEFTHOOK=0` skips only the local hook; CI
  still fails.
- To fix a flagged commit: `git rebase -i <base-sha>`, drop the matching
  lines, then force-push.

Maintainers: mark the GitHub check **Check commit attribution** as required
in branch protection; the workflow cannot enable that itself.
