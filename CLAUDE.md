...
- **Stack**: SwiftUI, iOS 26+, iPhone + iPad. SSH via the repository-local `Packages/HeelerSSH` (libssh2 + OpenSSL), terminal rendering via the pinned libghostty-spm `GhosttyTerminal` product. See ADR 0001 (native stack), ADR 0003 (iOS 26 target raise), and ADR 0004 (terminal engine).
...
- The herdr API socket serves **one request per connection** (read one line, write one line, close). Only `events.subscribe` and `pane.graphics.stream` keep the connection open. Plan channel usage accordingly. (Re-verified live on 0.8.0 for plain requests and `events.subscribe`: a second write on a served connection gets `EPIPE`. `pane.graphics.stream` was not re-tested.)
