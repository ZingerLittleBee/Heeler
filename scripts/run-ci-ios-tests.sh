...
if grep -q 'skipped:' "$transport_behavior_log" \
    || ! grep -q "Test run with 26 tests in 1 suite passed" "$transport_behavior_log"; then
    echo "The mandatory HeelerSSH Transport suite did not execute all twenty-six tests" >&2
    exit 1
fi
